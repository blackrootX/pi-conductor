...
describe("AgentRegistry", () => {
  ...
  describe("resolveByRole", () => {
    it("resolves agent by role when unique match", async () => {
      await registry.loadAll();

      // "project" role is unique in the test fixtures
      const result = await registry.resolveByRole("project");
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.agent.role).toBe("project");
    ...
    it("returns error for unknown role", async () => {
      ...
      const result = await registry.resolveByRole("nonexistent-role-xyz");
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.code).toBe("AGENT_NOT_FOUND_BY_ROLE");
        expect(result.error.requested.role).toBe("nonexistent-role-xyz");
      }
    });
    ...
    it("is case-insensitive", async () => {
      ...
      // "project" role is unique, so this won't trigger ambiguity
      const result = await registry.resolveByRole("PROJECT");
      expect(result.success).toBe(true);
    });

    it("returns ambiguity when multiple agents have the same role", async () => {
      // Create temp agents with same role but different IDs
      const tempDir = await createTempAgentsDir({
        "alpha-task.md": `---
name: Alpha Task Handler
role: handler
---
Alpha handler prompt.`,
        "beta-task.md": `---
name: Beta Task Handler
role: handler
---
Beta handler prompt.`,
      });
      ...
      const tempRegistry = new AgentRegistry({
        ...
        includeUser: true,
        includeProject: false,
        userAgentsPath: tempDir,
      });

      await tempRegistry.loadAll();

      const result = await tempRegistry.resolveByRole("handler");
      expect(result.success).toBe(false);
...
describe("deterministic resolution", () => {
  it("resolveByRole prefers higher priority within the same source when ambiguity allowed", async () => {
...
Beta reviewer prompt.`,
    ...
    await registry.loadAll();
    // With allowAmbiguous: true, auto-selects best match
    const result = await registry.resolveByRole("reviewer", { allowAmbiguous: true });
  ...
  it("resolveByCapability uses deterministic fallback after precedence when ambiguity allowed", async () => {
    const userAgentsPath = await createTempAgentsDir({
      "zeta-capability.md": `---
...
Alpha capability prompt.`,
    ...
    await registry.loadAll();
    // With allowAmbiguous: true, auto-selects best match
    const result = await registry.resolveByCapability("audit", { allowAmbiguous: true });
...
// ============================================================================
// Concurrent Loading Tests (Race Condition Fix)
// ============================================================================

describe("AgentRegistry concurrent loading", () => {
  it("handles concurrent loadAll() calls safely", async () => {
    const registry = new AgentRegistry({
      cwd: fixturesDir,
      includeBuiltIn: true,
      includeUser: true,
    ...
    // Fire multiple loadAll() calls concurrently
    ...
    // Registry should be fully loaded
    expect(registry.isLoaded()).toBe(true);

    // All methods should return complete state
    const agents = await registry.listAgents();
    expect(agents.length).toBeGreaterThan(0);
  ...
  it("handles concurrent listAgents() and resolveByRole() during loadAll()", async () => {
    const registry = new AgentRegistry({
      cwd: fixturesDir,
      includeBuiltIn: true,
      includeUser: true,
    ...
    // Start loadAll() and immediately fire concurrent reads
    const loadPromise = registry.loadAll();

    // These should all wait for loadAll() to complete and get complete state
    const [agents, count, roleResult] = await Promise.all([
      registry.listAgents(),
      registry.count(),
      registry.resolveByRole("planner", { allowAmbiguous: true }),
    ]);

    await loadPromise;

    // All should return complete data (no partial state)
    expect(agents.length).toBeGreaterThan(0);
    expect(count).toBe(agents.length);
    expect(await registry.count()).toBe(agents.length);
  });

  it("multiple callers hitting ensureLoaded() concurrently get same complete state", async () => {
    const registry = new AgentRegistry({
      cwd: fixturesDir,
      includeBuiltIn: true,
      includeUser: true,
    ...
    // Simulate multiple async callers hitting listAgents() around startup
    ...
    // All callers should get the same complete state
    for (const [agents, count, roles] of results) {
      expect(agents.length).toBeGreaterThan(0);
      expect(count).toBe(agents.length);
    ...
    const firstCount = results[0][1];
    for (const [, count] of results) {
      expect(count).toBe(firstCount);
    }
  });

  it("does not reload while a load is in progress", async () => {
    const registry = new AgentRegistry({
      cwd: fixturesDir,
      includeBuiltIn: true,
      includeUser: true,
    ...
    // Override _doLoad to track when it's called
    const originalDoLoad = registry["_doLoad"].bind(registry);
    registry["_doLoad"] = async () => {
      timestamps.push(Date.now());
      await originalDoLoad();
    };
    ...
    // _doLoad should only be called once (not 3 times)
...
describe("AgentRegistry ambiguity handling", () => {
  let registry: AgentRegistry;
  ...
  it("resolveByRole returns ambiguity when multiple matches exist", async () => {
    // Create a registry with multiple agents having the same role
    const tempDir = await createTempAgentsDir({
      "alpha-planner.md": `---
name: Alpha Planner
role: planner
...
Gamma planner prompt.`,
    ...
    // Without allowAmbiguous, should return ambiguity
    const result = await tempRegistry.resolveByRole("planner");
    expect(result.success).toBe(false);
    expect("ambiguous" in result && result.ambiguous).toBe(true);
    if ("ambiguous" in result && result.ambiguous) {
      expect(result.matches.length).toBe(3);
      expect(result.error.code).toBe("AMBIGUOUS_RESOLUTION");
    }
  });

  it("resolveByRole with allowAmbiguous selects best match", async () => {
    const tempDir = await createTempAgentsDir({
      "alpha-planner.md": `---
name: Alpha Planner
role: planner
...
Beta planner prompt.`,
    ...
    const result = await tempRegistry.resolveByRole("planner", { allowAmbiguous: true });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.agent.name).toBe("Beta Planner"); // Higher priority wins
    }
  });

  it("resolveByCapability returns ambiguity when multiple matches exist", async () => {
    const tempDir = await createTempAgentsDir({
      "audit-alpha.md": `---
name: Audit Alpha
capabilities:
...
Beta audit prompt.`,
    ...
    const result = await tempRegistry.resolveByCapability("audit");
    expect(result.success).toBe(false);
    expect("ambiguous" in result && result.ambiguous).toBe(true);
    if ("ambiguous" in result && result.ambiguous) {
      expect(result.matches.length).toBe(2);
  ...
  it("resolve with role and allowAmbiguous selects from matches", async () => {
    const tempDir = await createTempAgentsDir({
      "alpha-specialist.md": `---
...
priority: 5
---
Alpha specialist prompt.`,
      "beta-specialist.md": `---
name: Beta Specialist
role: specialist
priority: 10
---
Beta specialist prompt.`,
    });
    ...
    const result = await tempRegistry.resolve({ role: "specialist", options: { allowAmbiguous: true } });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.agent.name).toBe("Beta Specialist");
      expect(result.resolvedBy).toBe("role");
    }
  });

  it("returns ambiguity for ambiguous role even if ID doesn't exist", async () => {
    const tempDir = await createTempAgentsDir({
      "alpha-dev.md": `---
name: Alpha Developer
role: developer
---
Alpha developer prompt.`,
      "beta-dev.md": `---
name: Beta Developer
role: developer
---
Beta developer prompt.`,
    });
    ...
    // Query with non-existent ID but matching role
    const result = await tempRegistry.resolve({ id: "non-existent", role: "developer" });
    // Should return ambiguity from role resolution, not "not found"
    expect(result.success).toBe(false);
    expect("ambiguous" in result && result.ambiguous).toBe(true);
  });

  it("diagnostics include RESOLUTION_AMBIGUOUS for ambiguous resolutions", async () => {
    const tempDir = await createTempAgentsDir({
      "alpha-tag.md": `---
name: Alpha Tagger
role: tagger
...
Beta tagger prompt.`,
    ...
    await tempRegistry.loadAll();
    await tempRegistry.resolveByRole("tagger");

    const diagnostics = await tempRegistry.listDiagnostics();
    const ambiguityDiags = diagnostics.filter((d) => d.code === "RESOLUTION_AMBIGUOUS");
    expect(ambiguityDiags.length).toBeGreaterThan(0);
  });

  it("preferSource option filters matches before ambiguity check", async () => {
    const tempDir = await createTempAgentsDir({
      "alpha-task.md": `---
name: Alpha Task Manager
role: manager
---
Alpha manager prompt.`,
      "beta-task.md": `---
name: Beta Task Manager
role: manager
---
Beta manager prompt.`,
    });
    ...
    // Without preferSource, should be ambiguous
    const ambiguousResult = await tempRegistry.resolveByRole("manager");
    expect(ambiguousResult.success).toBe(false);
    expect("ambiguous" in ambiguousResult && ambiguousResult.ambiguous).toBe(true);
  });

  it("single match is not ambiguous", async () => {
    const tempDir = await createTempAgentsDir({
      "unique-role.md": `---
name: Unique Role
role: unique
...
Unique role prompt.`,
    ...
    const result = await tempRegistry.resolveByRole("unique");
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.agent.name).toBe("Unique Role");
    }
  });

  it("unmatched query returns AGENT_NOT_FOUND, not ambiguity", async () => {
    const result = await registry.resolveByRole("non-existent-role-xyz");
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.code).toBe("AGENT_NOT_FOUND_BY_ROLE");
